...
def execute_se_reproduire_entite(
    ...
    if entite.est_enceinte:
        # Execution action
        bebe = entite.se_reproduire(cases_voisines_vides) # L'entite a changé de position
...
def execute_s_alimenter_requin(
    ...
    if len(cases_voisines_poissons) > 0:
        # Execution action
...
def test():
    ...
    for ligne in grille_demo.grille: print(ligne)
    print(execute_se_deplacer_entite(poisson_demo, poisson_demo.position, grille_demo.cases_libres(poisson_demo.position), grille_demo, deja_agis))
    for ligne in grille_demo.grille: print(ligne)
    print(execute_se_deplacer_entite(requin_demo, requin_demo.position, grille_demo.cases_libres(requin_demo.position), grille_demo, deja_agis))
    for ligne in grille_demo.grille: print(ligne)
    poisson_demo._est_enceinte = True
    print(execute_se_reproduire_entite(poisson_demo, poisson_demo.position, grille_demo.cases_libres(poisson_demo.position), grille_demo, deja_agis))
    for ligne in grille_demo.grille: print(ligne)
    print(execute_se_reproduire_entite(requin_demo, requin_demo.position, grille_demo.cases_libres(_demo.position), grille_demo, deja_agis))
    for ligne in grille_demo.grille: print(ligne)
