...
def dessiner_bouton(rect, texte, survol):
    ...
    ecran.blit(label, (rect.x + (rect.width - label.get_width()) // 2, rect.y + (rect.height - label.get_height()) // 2))
...
def afficher_menu():
    ...
    while en_menu:
        ...
        ecran.blit(police.render(f"Poissons : {nb_poissons}", True, COULEUR_TEXTE), (LARGEUR // 2 - 80, HAUTEUR // 2 - 30))
        ...
        ecran.blit(police.render(f"Requins : {nb_requins}", True, COULEUR_TEXTE), (LARGEUR // 2 - 80, HAUTEUR // 2 + 30))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if moins_poisson.collidepoint(souris): nb_poissons = max(0, nb_poissons - 1)
                ...
                if btn_lancer.collidepoint(souris):
                    ...
                    en_menu = False
                if btn_quitter.collidepoint(souris): video.release(); pygame.quit(); exit()
...
def dessiner_monde(monde, tour, pause=False):
    ...
    titre_legende = petite_police.render("Population (âge croissant)", True, (255, 255, 255))
...
def afficher_ecran_fin():
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT: pygame.quit(); exit()
            if event.type == pygame.MOUSEBUTTONDOWN:
                if btn_rejouer.collidepoint(souris): afficher_menu(); lancer_simulation(); return
                if btn_quitter.collidepoint(souris): pygame.quit(); exit()
        pygame.display.flip()
        horloge.tick(30)
